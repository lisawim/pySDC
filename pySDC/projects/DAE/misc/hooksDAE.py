...
class LogGlobalErrorPreSweepDiff(Hooks):
    def pre_sweep(self, step, level_number):
        ...
        e_global = abs(upde[:].diff - L.u[-1][:].diff)
        ...
        self.add_to_stats(
            ...
            type="e_global_differential_pre_sweep",
            value=e_global,
        )


class LogGlobalErrorPreSweepAlg(Hooks):
    def pre_sweep(self, step, level_number):
        super().pre_sweep(step, level_number)
        ...
        self.add_to_stats(
            ...
            iter=step.status.iter,
            sweep=L.status.sweep,
            type="e_global_algebraic_pre_sweep",
...
class LogGlobalErrorPostSweepDiff(Hooks):
    """
    Hook class to log the global error in the differential variable and its derivative after each sweep.
    """
...
class LogGlobalErrorPostSweepAlg(Hooks):
    """
    Hook class to log the global error in the algebraic variable and its derivative after each sweep.
    """
