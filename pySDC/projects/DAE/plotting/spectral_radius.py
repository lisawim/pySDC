...
from pySDC.projects.DAE.run.study_embedding_linear import get_color_embedding
...
def compute_normality_deviation(A):
    ...
    The maximum norm of the commutator measures how far A is from being normal.
    ...
    -------
    deviation : float
        The maximum norm of the commutator A*A - AA*.
    """
...
def get_iteration_matrices_ordered_by_nodes(dt, num_nodes, Qmat, QImat, problem_name, sweeper_type, eps=0.0):
    r"""
    ...
    if sweeper_type in ["embeddedDAE", "fullyImplicitDAE", "SPP"]:
...
def get_iteration_matrices_ordered_by_vars(dt, num_nodes, Qmat, QImat, problem_name, sweeper_type, eps=0.0):
    r"""
    Returns iteration matrix for different SDC methods.
...
def plot_spectral_radius(
    ...
    sweeper_type = "embeddedDAE"#"semiImplicitDAE"#"constrainedDAE"
    ...
    plot_name = f"sr_{sweeper_type}.png"
...
def plot_spectral_radius_sdc_spp_and_sdc_e_and_sdc_c(journal="Springer_Scientific_Computing"):
    ...
    for s, sweeper_type in enumerate(sweeper_types):
        ...
        for e, eps in enumerate(eps_list):
            for q, QI in enumerate(QI_list):
                if eps > 0.0 and QI != "MIN-SR-NS":
                    continue

                print(f"{eps=}: Running for {sweeper_type} with {QI}..")
                key = f"constrainedDAE_{QI}"
                ...
                for num_nodes in num_nodes_list:
                    ...
                    spectral_radius = max(abs(np.linalg.eigvals(K)))
                    print(f"Its spectral radius is: {spectral_radius}\n")
                    spectral_radii.append(spectral_radius)

                color = colors[key] if eps == 0.0 else get_color_embedding(sweeper_type, e)
                if eps == 0.0:
...
def plot_spectral_radius_and_evd(problem_name="LINEAR-TEST", journal="Springer_Scientific_Computing"):
    figsize = figsize_by_journal(journal, scale=2.0, ratio=0.8)
    ...
    setup_mpl_spectrum(fontsize=8)
    colors, markers, _ = my_plot_style_config()

    gridspec_kw = {"width_ratios": [2, 1, 1], "wspace": 0.8, "hspace": 0.6}
    fig, axs = plt.subplots(3, 3, figsize=figsize, gridspec_kw=gridspec_kw)

    outer_gs = plt.GridSpec(3, 3, width_ratios=[2, 1, 1], wspace=0.7)[:3, 0]
    ...
    plot_name = f"sr_ev_{num_nodes=}_{dt_fix=}_{sweeper_type}.png"
    filename = "data" + "/" + f"{problem_name}" + "/" + plot_name
    file_path = Path(filename)
    file_path.parent.mkdir(parents=True, exist_ok=True)
...
def plot_evd(problem_name="LINEAR-TEST", journal="Springer_Scientific_Computing"):
    figsize = figsize_by_journal(journal, scale=1.0, ratio=0.66)

    sweeper_type = "embeddedDAE"
    # QI_list = ["IE", "EE", "LU", "MIN-SR-S", "MIN-SR-NS", "Picard"]
    QI_list = ["IE", "LU", "MIN-SR-S", "MIN-SR-NS"]
    ...
    for d, dt in enumerate(dt_list):
        ...
        plot_name = f"{d+1}_evd_{sweeper_type}_{dt=}.png"
    ...
    for num_nodes in num_nodes_list:
        ...
        plot_name = f"evd_{sweeper_type}_{num_nodes=}.png"
        filename = "data" + "/" + f"{problem_name}" + "/" + "evd" + "/" + plot_name
        file_path = Path(filename)
        file_path.parent.mkdir(parents=True, exist_ok=True)
...
if __name__ == "__main__":
    # plot_spectral_radius_and_evd(journal="BUW_thesis")
    # plot_error_norm(format="png")
    # plot_evd(journal="BUW_thesis")
    # plot_spectral_radius(journal="BUW_thesis")
    plot_spectral_radius_sdc_spp_and_sdc_e_and_sdc_c(journal="BUW_thesis")
