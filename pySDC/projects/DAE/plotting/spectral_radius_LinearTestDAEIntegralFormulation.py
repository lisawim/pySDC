...
def plot():
    r"""
    Function plots the spectral radius for ``LinearTestDAEConstrained``.
    """

    Path("data").mkdir(parents=True, exist_ok=True)
    Path(f"data/LinearTestDAEConstrained").mkdir(parents=True, exist_ok=True)
    ...
    fig.savefig(f"data/LinearTestDAEConstrained/SR_IterMatrix_QI={QI}_M={M}_{quad_type}.png", dpi=300, bbox_inches='tight')
    plt.close(fig)
