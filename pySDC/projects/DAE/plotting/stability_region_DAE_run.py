...
from pySDC.projects.DAE import computeSolution, getColor, getLabel, get_linestyle, getMarker, Plotter
...
if __name__ == "__main__":
    ...
    resolution = 400
    x = np.linspace(-200.0, 200.0, num=resolution)
    y = np.linspace(-200.0, 200.0, num=resolution)
                ...
                for q, QI in enumerate(QI_list):
                    ...
                    for i in range(resolution):
                        for j in range(resolution):
                            ...
                            kwargs = {"lamb_diff": lamb, "nsweeps": num_sweeps, "maxiter": 1, "e_tol": -1}
                            ...
                            solution_stats = computeSolution(
                                ...
                                hookClass=[LogSolution],
                                eps=eps,
                                **kwargs,
                            )

                            u_val = [me[1] for me in get_sorted(solution_stats, type="u", sortby="time")][-1]

                            rho[i, j] = np.linalg.norm(u_val.flatten())
