...
class LogPWMSignal(Hooks):
    """
    Hook class to log the value of the PWM signal at the output generated by the sweeper after each time step.
    """
    ...
    def post_step(self, step, level_number):
        ...
        # log the value of the PWM signal at the end of each time step
        self.add_to_stats(
...
class BuckConverterDAE(ProblemDAE):
    ...
    def set_pwm_signal(self, t):
        """
        Routine to evaluate the value of the PWM signal at time t.

        Parameters
        ----------
        t : float
            Time at which the PWM signal should be evaluated.

        Returns
        -------
        pwm_value : float
            Value of the PWM signal at time t.
        """

        if 0 <= ((t / self.Tsw) % 1) <= self.duty:
            self.pwm_signal = self.Vs
        else:
            self.pwm_signal = 0.0
