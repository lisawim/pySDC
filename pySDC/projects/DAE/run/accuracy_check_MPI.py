...
def run(dt, num_nodes, use_MPI, semi_implicit, residual_type, index_case, initial_guess='spread', comm=None):
    ...
    if not semi_implicit:
        if use_MPI:
            from pySDC.projects.DAE import fully_implicit_DAE_MPI as sweeper

        else:
            from pySDC.projects.DAE import fully_implicit_DAE as sweeper

    else:
        if use_MPI:
            from pySDC.projects.DAE import SemiImplicitDAEMPI as sweeper

        else:
            from pySDC.projects.DAE import SemiImplicitDAE as sweeper

    if index_case == 1:
        from pySDC.projects.DAE import DiscontinuousTestDAE as problem

        t0 = 1.0
        Tend = 1.5

    elif index_case == 2:
        from pySDC.projects.DAE import simple_dae_1 as problem
