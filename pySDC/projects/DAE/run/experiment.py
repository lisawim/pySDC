...
# Exact solutions
def y_exact(t, lambda_):
    return np.exp(lambda_ * t)

def z_exact(t, lambda_):
    return y_exact(t, lambda_)
...
def eval_rhs(y, z, t, lambda_):
    eq1 = (lambda_ - 1) * y + z
    eq2 = -z + y
    rhs = np.array([eq1, eq2])
    return rhs

def integrate_function(coll_nodes, dt, M, Qmat, u_const, u_embed, lambda_):
    integral_const = np.zeros((2, M))
    integral_embed = np.zeros((2, M))

    for m in range(M):
        for j in range(M):
            f_const = eval_rhs(u_const[0, j], u_const[1, j], coll_nodes[j], lambda_)
            f_embed = eval_rhs(u_embed[0, j], u_embed[1, j], coll_nodes[j], lambda_)
...
def test_integrate(coll_nodes, dt, integral_const, integral_embed, M, Qmat, u_const, u_embed, lambda_):
    f1_const = np.array([eval_rhs(u_const[0, m], u_const[1, m], coll_nodes[m], lambda_)[0] for m in range(M)])
    ...
    f1_embed = np.array([eval_rhs(u_embed[0, m], u_embed[1, m], coll_nodes[m], lambda_)[0] for m in range(M)])
    f2_embed = np.array([eval_rhs(u_embed[0, m], u_embed[1, m], coll_nodes[m], lambda_)[1] for m in range(M)])
...
def solve_system_const(rhs, factor, u0, t, lambda_, newton_tol=1e-12, newton_maxiter=15):
    u = u0.copy()
    ...
    while n < newton_maxiter:
        ...
        f = eval_rhs(y, z, t, lambda_)

        g = np.array([y - factor * f[0] - rhs[0], f[1]])
        ...
        # Inverse of dg
        dg = np.array([[1 - factor * (lambda_ - 1), -factor], [1, -1]])
...
def solve_system_embed(rhs, factor, u0, t, lambda_, newton_tol=1e-12, newton_maxiter=15):
    u = u0.copy()
    ...
    while n < newton_maxiter:
        ...
        g = np.array([y - factor * f[0] - rhs[0], -factor * f[1] - rhs[1]])
        ...
        # Inverse of dg
        dg = np.array([[1 - factor * (lambda_ - 1), -factor], [-factor, factor]])
...
if __name__ == "__main__":
    ...
    y0 = y_exact(t0, lambda_)
    z0 = z_exact(t0, lambda_)
    ...
    for dt in dt_values:
        ...
        while k < maxiter:
            ...
            # Integrate
            integral_const, integral_embed = integrate_function(coll_nodes, dt, M, Qmat, u_const, u_embed, lambda_)

            test_integrate(coll_nodes, dt, integral_const, integral_embed, M, Qmat, u_const, u_embed, lambda_)
            ...
            for m in range(M):
                ...
                for j in range(0, m):
                    f_const = eval_rhs(u_const_new[0, j], u_const_new[1, j], coll_nodes[j], lambda_)
                    f_embed = eval_rhs(u_embed_new[0, j], u_embed_new[1, j], coll_nodes[j], lambda_)

                    b_const[0] += dt * QImat[m, j] * f_const[0]
                    b_embed[:] += dt * QImat[m, j] * f_embed

                factor = dt * QImat[m, m]
                u_const_new[:, m] = solve_system_const(b_const, factor, u_const[:, m], coll_nodes[m], lambda_)
                u_embed_new[:, m] = solve_system_embed(b_embed, factor, u_embed[:, m], coll_nodes[m], lambda_)
        ...
        y_ex = y_exact(t + dt, lambda_)
        z_ex = z_exact(t + dt, lambda_)
        u_ex = np.array([y_ex, z_ex])

        manifold_ex = eval_rhs(y_ex, z_ex, t + dt, lambda_)[1]
        ...
        manifold_const = abs(max([eval_rhs(u_const[0, m], u_const[1, m], coll_nodes[m], lambda_)[1] for m in range(M)]))
        manifold_embed = abs(max([eval_rhs(u_embed[0, m], u_embed[1, m], coll_nodes[m], lambda_)[1] for m in range(M)]))
