...
def finalize_plot(plotter, num_nodes, problem_type, problem_name, QI):
    ...
    plotter.set_shared_legend(loc="lower center", bbox_to_anchor=(0.5, -0.17), ncol=4, fontsize=22)
...
if __name__ == "__main__":
    ...
    dt_list_short = dt_list[2:6]

    case = 4

    hook_class = get_hooks(k=case, hook_for="iteration")

    problem_type = "constrainedDAE"#"semiImplicitDAE"#"constrainedDAE"
    eps = 0.0
    ...
    for q, QI in enumerate(QI_list):
        ...
        for dt in dt_list:
            ...
            err_diff_values = np.array([me[1] for me in get_sorted(solution_stats, type=f"e_global_differential_post_iteration", sortby="iter")])
            err_alg_values = np.array([me[1] for me in get_sorted(solution_stats, type=f"e_global_algebraic_post_iteration", sortby="iter")])
        ...
        for k in range(maxiter):
            order_plotter = plot_result(
                order_plotter,
                dt_list_short,
                [4e-1 * dt ** (k + 2) for dt in dt_list_short],
                0,
                "black",
                None,  # marker,
                None,  # markersize,
