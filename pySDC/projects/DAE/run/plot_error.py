...
def plot_algebraic_error_vs_iteration(
    ...
    for sweeper_type_QI_pair in sweeper_type_QI_pair_list:
        ...
        if (sweeper_type, QI) != ("fullyImplicitDAE", "Picard"):
            ...
            ax.semilogy(
                ...
                markersize=2.9,
                markeredgewidth=0.4,
                alpha=1.0 if (sweeper_type, QI) in fi_sdc_stiff_list else 0.4,
                label=label,
            )
    ...
    plot_name = "error_z_vs_iterations_highlight_fi_sdc_nonstiff.png"#f"Fig7.png"
    filename = "data" + "/" + f"{problem_name}" + "/" + plot_name
    file_path = Path(filename)
    file_path.parent.mkdir(parents=True, exist_ok=True)
...
if __name__ == "__main__":
    ...
    plot_algebraic_error_vs_iteration(1e-3, 6, problem_name="ANDREWS-SQUEEZER")