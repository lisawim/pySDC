...
class SwitchEstimator(ConvergenceController):
    ...
    def get_new_step_size(self, controller, S):
        ...
        if S.status.iter == S.params.maxiter and self.params.count_switches < np.shape(V_ref)[0]:
                ...
                if vC_switch[0] * vC_switch[-1] < 0:
                    ...
                    if L.time < self.params.t_switch < L.time + L.dt:
                        ...
                        if not np.isclose(self.params.t_switch - L.time, L.dt, atol=self.params.tol):
                            ...
                            dt_switch = self.params.t_switch - L.time
                            L.prob.params.set_switch[self.params.count_switches] = self.params.switch_detected
                            L.prob.params.t_switch[self.params.count_switches] = self.params.t_switch
                            controller.hooks.add_to_stats(
                                ...
                                type='switch{}'.format(self.params.count_switches + 1),
    ...
    def post_step_processing(self, controller, S):
        ...
        if self.params.switch_detected_step:
            if L.prob.params.set_switch[self.params.count_switches] and L.time + L.dt >= self.params.t_switch:
                self.params.count_switches += 1
                self.params.t_switch = None
                self.params.switch_detected_step = False

                # L.status.dt_new = self.params.dt_initial
